...
*/
// Helper function to check if the k-plex is connected
bool EnuBundle::isConnected()
{
	if (P.getSize() < 2)
		return true; // A single vertex or empty set is trivially connected

	vector<bool> visited(bn, false);
	stack<ui> s;
	ui start = P.get(0);
	s.push(start);
	visited[start] = true;
	...
	while (!s.empty())
	{
		...
		for (ui i = bstart[v]; i < bstart[v + 1]; i++)
		{
			ui nei = bedges[i];
			if (P.contains(nei) && !visited[nei])
			{
				visited[nei] = true;
				s.push(nei);
				visitedCount++;
	...
	return visitedCount == P.getSize();
}
void EnuBundle::print_final_metrics()
{
	...
	std::cout << "\n-----------------------------------------" << endl;
	std::cout << "Final max results:" << endl;
	std::cout << "Highest final average degree is " << final_max_average_degree << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : final_max_average_degree_sol)
	{
		std::cout << node << " ";
	}
	std::cout << "for k = " << kad << std::endl;
	std::cout << "Highest final internal density is " << final_max_internal_density << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : final_max_internal_density_sol)
	{
		std::cout << node << " ";
	}
	std::cout << "for k = " << kid << std::endl;
	std::cout << "Highest final cut ratio is " << final_max_cut_ratio << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : final_max_cut_ratio_sol)
	{
		std::cout << node << " ";
	}
	std::cout << "for k = " << kcr << std::endl;
	std::cout << "Highest final conductance is " << final_max_conductance << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : final_max_conductance_sol)
	{
		std::cout << node << " ";
	}
	std::cout << "for k = " << kc << std::endl;
	std::cout << "Highest final modularity is " << final_max_modularity << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : final_max_modularity_sol)
	{
		std::cout << node << " ";
	}
	std::cout << "for k = " << km << std::endl;
	//std::cout << mmm << " " << nmm << " " << nbm << endl;
}
void EnuBundle::print_metrics()
{
	std::cout << "Highest average degree is " << max_average_degree << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : max_average_degree_sol)
	{
		std::cout << node << " ";
	}
	std::cout << std::endl;
	std::cout << "Highest internal density is " << max_internal_density << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : max_internal_density_sol)
	{
		std::cout << node << " ";
	}
	std::cout << std::endl;
	std::cout << "Highest cut ratio is " << max_cut_ratio << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : max_cut_ratio_sol)
	{
		std::cout << node << " ";
	}
	std::cout << std::endl;
	std::cout << "Highest conductance is " << max_conductance << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : max_conductance_sol)
	{
		std::cout << node << " ";
	}
	std::cout << std::endl;
	std::cout << "Highest modularity is " << max_modularity << ", and ";
	std::cout << "corresponding kplex: ";
	for (ui node : max_modularity_sol)
	{
		std::cout << node << " ";
	}
	std::cout << std::endl;
...
void EnuBundle::clear_metrics()
{
	kplex.clear();
	max_average_degree = 0;
	max_average_degree_sol.clear();
	max_internal_density = 0;
	max_internal_density_sol.clear();
	max_cut_ratio = 0;
	max_cut_ratio_sol.clear();
	max_conductance = 0;
	max_conductance_sol.clear();
	max_modularity = 0;
	max_modularity_sol.clear();
}

void EnuBundle::calculateMaxAvgDegreeKplex()
{
	ui beg;
	ui en;
	double ne = 0;
	...
	for (ui node : kplex)
	{
		...
		en = pstart[node + 1];
		visited.insert(node);
		for (ui index = beg; index < en; index++)
		{
			if ((kplex.find(edges[index]) != kplex.end()) && (visited.find(edges[index]) != visited.end()))
			{
				ne++;
			}
			// visited.insert(edges[index]);
		}
	}
	nv = kplex.size();
	average_degree = (2 * ne) / nv;
	/*std::cout << "kplex: ";
	for (ui node: kplex){
		std::cout << node << " ";
	}
	std::cout << endl;
	std::cout << "average degree: " << average_degree << endl;*/
	if (average_degree > max_average_degree)
	{
		max_average_degree = average_degree;
		max_average_degree_sol = kplex;
	}
	if (average_degree > final_max_average_degree)
	{
		final_max_average_degree = average_degree;
		final_max_average_degree_sol = kplex;
		kad = k;
	}
	/*std::cout << "---------------\n";
	std::cout << "Average degree is " << average_degree << ", corresponding k-plex: ";
	for (ui node : kplex)
	{
		std::cout << node << " ";
	}
...
void EnuBundle::calculateMaxInternalDensityKplex()
{
	ui beg;
	ui en;
	double ne = 0;
	...
	nv = kplex.size();
	internal_density = (2 * ne) / (nv * (nv - 1));
	/*std::cout << "---------------\n";
	std::cout << "intenral density: " << internal_density << ", and corresponding k-plex: ";
	for (ui node: kplex){
		std::cout << node << " ";
	std::cout << endl;*/
	if (internal_density > max_internal_density)
	{
		max_internal_density = internal_density;
		max_internal_density_sol = kplex;
	}
	if (internal_density > final_max_internal_density)
	{
		final_max_internal_density = internal_density;
		final_max_internal_density_sol = kplex;
		kid = k;
	}
}

void EnuBundle::calculateMaxCutRatioKplex()
{
	ui beg;
	ui en;
	//ui number_of_elements = 0;
	//ui ne = 0;
	double nb = 0;
	double nv = 0;
	double cut_ratio;
	...
	for (ui node : kplex)
	{
		...
		for (ui index = beg; index < en; index++)
		{
			if (kplex.find(edges[index]) == kplex.end())
			{
				//ne++;
				nb++;
			}
		...
		// std::cout << "vertex: " << node << ", number of edges = " << ne << endl;
		// std::cout << "number of neighbors: " << number_of_elements << endl;
		/*if (ne != number_of_elements)
		{
			nb++;
		}*/
	}
	nv = kplex.size();
	cut_ratio = 1 - (nb / (nv * (n - nv)));
	/*std::cout << "---------------\n";
	std::cout << "cut ratio: " << cut_ratio << ", and corresponding k-plex: ";
	for (ui node: kplex){
		std::cout << node << " ";
	}
	std::cout << endl;*/
	...
	{
		max_cut_ratio = cut_ratio;
		max_cut_ratio_sol = kplex;
	}
	if (cut_ratio > final_max_cut_ratio)
	{
		final_max_cut_ratio = cut_ratio;
		final_max_cut_ratio_sol = kplex;
		kcr = k;
	}
}
void EnuBundle::calculateMaxConductanceKplex()
{
	ui beg;
	ui en;
	//ui number_of_elements = 0;
	...
	std:: cout << endl;*/
	conductance = 1 - (nb / (nb + (2 * nm)));
	if (conductance > max_conductance)
	{
		max_conductance = conductance;
		max_conductance_sol = kplex;
	}
	if (conductance > final_max_conductance)
	{
		final_max_conductance = conductance;
		final_max_conductance_sol = kplex;
		kc = k;
	}
}

void EnuBundle::calculateMaxModularityKplex()
{
	ui mm = m / 2;
	ui beg;
	ui en;
	...
	modularity = (nm/mm) - pow((((2 * nm) + nb) / (2 * mm)), 2);
	if (modularity > max_modularity)
	{
		max_modularity = modularity;
		max_modularity_sol = kplex;
	}
	if (modularity > final_max_modularity)
	{
		final_max_modularity = modularity;
		final_max_modularity_sol = kplex;
		km = k;
		/*mmm = mm;
		nmm = nm;
		nbm = nb;*/
...
int EnuBundle::readBinaryGraph(const char *filepath)
{
	...
	fclose(f);
	ui minDegree = UINT_MAX; // Initialize with the maximum possible value
	for (ui i = 0; i < n; i++)
	{
...
void EnuBundle::checkSolution()
{
	...
	if (isConnected())
	{
		...
		for (ui i = 0; i < P.getSize(); i++)
		{
			kplex.insert(bID[P.get(i)]);
		}
		calculateMaxAvgDegreeKplex();
		calculateMaxInternalDensityKplex();
		calculateMaxCutRatioKplex();
		calculateMaxConductanceKplex();
		calculateMaxModularityKplex();
		// #ifdef SHOWSOL
		if (v)
		{
			printf("Sol: ");
...
void EnuBundle::branch()
{
	...
	*/
	...
	if (minu != UINT_MAX && neiInG[minu] + k < Cand.getSize() + P.getSize())
	{ // The whole graph can not be a k-plex
...
void EnuBundle::enumPlex(ui _k, uli _maxsec, ui _quiete, ui _v, ui _lb)
{
	startclk = clock();
	k = _k;
	lb = _lb;
	...
	//}
	enumclk = clock();
	printf("Number of %u-kplex larger than %u:  %u\n", k, lb, cntplex);
