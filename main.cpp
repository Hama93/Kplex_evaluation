...
int main(int argc, char **argv)
{
    ...
    args::ValueFlag<int> Verb(parser, "verb", "print solutions or not", {'v', "verb"}, 0);
	...
	ui kmax = enbundle.getKmax();
	for (ui k = kmax - 1; k > 0; k--)
	{
		printf("\n------------------------------\nResult for k = %u:\n------------------------------\n", k);
		enbundle.enumPlex(k, maxsec, isquiete, v, lb);
	}
	enbundle.print_final_metrics();
	//_CrtDumpMemoryLeaks();
	return 0;
}
